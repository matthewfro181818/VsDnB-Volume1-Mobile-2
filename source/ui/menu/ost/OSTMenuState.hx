...
case CATEGORY:
 if (FlxG.keys.justPressed.UP #if mobileC || virtualPad.buttonUp.justPressed # || FlxG.mouse.wheel > 0) {
toggleCategorySelect(false);
 changeSongSelection(categorySongsCount - currentSongSelected);

 if (FlxG.keys.justPressed.DOWN #if mobileC || virtualPad.buttonDown.justPressed # || FlxG.mouse.wheel < 0) {
toggleCategorySelect(false);
...
arrowRight.scale.set(1, 1);
 ...
 case SONG:
 if (FlxG.keys.justPressed.UP #if mobileC || virtualPad.buttonUp.justPressed #) {
...
changeSongSelection(-1);
 ...
 if (FlxG.keys.justPressed.DOWN #if mobileC || virtualPad.buttonDown.justPressed #) {
...
if (FlxG.keys.pressed.LEFT #if mobileC || virtualPad.buttonLeft.pressed #) {
...
arrowRightVariation.color = FlxColor.WHITE;
 ...
 if (FlxG.keys.justPressed.ENTER #if mobileC || virtualPad.buttonA.justPressed # && selectedSongOption.getVariation(selectedVariation) != turnTable.currentPlayData); {
...
this.remove(icon);
 ...
 // Trim each text so it cuts off before the of the width.
 ...
 // Reposition the icon to be at the of the name text.
 if (icon != null) {
