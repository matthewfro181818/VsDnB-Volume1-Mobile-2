...
case TFun(args, ret):
							...
							var constArgs = [;
								for (arg in args) {name: arg.name, opt: arg.opt, type: Context.toComplexType(arg.t)
							];
...
name: 'scriptCall',
			...
			pos: cls.pos,
			kind: FFun({
args: [ {name: 'funcName', type: Context.toComplexType(Context.getType('String'))}, {
name: 'funcArgs',
...
// Sometimes a child version needs to be skipped but the parent version doesn't.
					...
					// Example: A child function /*override_removed*/ can be inline when the parent isn't.
