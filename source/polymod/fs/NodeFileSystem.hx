...
");
		...
		jsCode.push("
");
		jsCode.push("		var entries = _nodefs.readdirSync(path, {
withFileTypes:true
} );");
		...
		jsCode.push("function exists(path) {
return _nodefs.existsSync(path);
}");
		jsCode.push("function getStats(path) {
return exists(path) ? _nodefs.statSync(path) : null;
}");
		jsCode.push("function isDirectory(path) {
var stats = getStats(path); return stats != null && stats.isDirectory();
}");
		jsCode.push("function getFileContent(path) {
return exists(path) ? _nodefs.readFileSync(path, {encoding:'utf8', flag:'r'}) : '';
}");
		jsCode.push("function getFileBytes(path) {
return exists(path) ? Uint8Array.from( _nodefs.readFileSync(path) ) : null;
}");
		jsCode.push("function readDirectory(path) {
return getDirectoryContents(path, false, [])
}");
		jsCode.push("function readDirectoryRecursive(path) {
return getDirectoryContents(path, true, [])
}");
