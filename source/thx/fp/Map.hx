...
case [Tip, _]: that;
 ...
 > (r : Map<K, V>).size()):
	...
	static function deleteFindMin<K, V>(map:Map<K, V>)
		return switch map {
case Bin(_, k, x, Tip, r): {k: k, x: x, t: r};
			case Bin(_, k, x, l, r):
				var t = deleteFindMin(l); {k: t.k, x: t.x, t: balance(k, x, t.t, r)}
			case Tip:
	...
	static function deleteFindMax<K, V>(map:Map<K, V>)
		return switch map {
case Bin(_, k, x, l, Tip): {k: k, x: x, t: l};
			case Bin(_, k, x, l, r):
				var t = deleteFindMax(r); {k: t.k, x: t.x, t: balance(k, x, l, t.t)}
			case Tip:
