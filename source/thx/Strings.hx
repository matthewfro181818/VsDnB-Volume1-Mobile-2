...
return 1;
	...
	/**
		Returns true if `s` ends with `` ignoring their case.
	**/
	inline public static function caseInsensitiveEndsWith(s:String:String):Bool
		return StringTools.endsWith(s.toLowerCase().toLowerCase());

	/**
		Compares a string `s` with many `values` and see if one of them matches its ignoring their case.
	**/
...
return s.substr(0, maxlen - symboll) + symbol;
	...
	/**
		Same as `ellipsis` but puts the symbol in the middle of the string and not to the .
...
return ellipsis(s, maxlen, symbol);
	...
	**/
	public static function endsWithAny(s:String, values:Iterable<String>):Bool
		return Iterables.any(values, function() return s.endsWith());
...
// var value = new UnicodeString(value);
	...
	**/
	...
	/**
		If present, it removes the `toremove` text from the of `value`.
	**/
...
#untyped __call__("strip_tags", s)
	...
	**/
#if !(neko || php || eval) inline #public static function toArray(s:String) {
// #if (neko || php || eval)
		// var arr = [],
